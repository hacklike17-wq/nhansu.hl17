...
import { Shield, ShieldCheck, ShieldAlert, Pencil, Plus, Check, Users } from 'lucide-react'
...
export default function PhanQuyenPage() {
  ...
  const openEmpEdit = (emp: any) => {
    setEditEmpId(emp.id)
    setEmpRole((emp.user?.role ?? 'employee') as UserRole)
    setEmpOverrides(new Set())
  }
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {employees.map((emp: any) => {
                const role = ROLE_STYLE[emp.user?.role ?? 'employee'] || ROLE_STYLE.employee
                  ...
                  <tr key={emp.id} className="border-b border-gray-50 hover:bg-blue-50/20">
                    ...
                    <td className="px-4 py-2 text-center">
                      <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded border text-[10px] font-semibold ${role.cls}`}>
                        {role.icon} {groups.find((g: PermissionGroup) => g.name === (emp.user?.role ?? 'employee'))?.label || (emp.user?.role ?? 'employee')}
                      </span>
                    </td>
                    <td className="px-4 py-2 text-center">
          ...
          <div onClick={e => e.stopPropagation()} className="bg-white rounded-2xl border border-gray-200 shadow-2xl w-[700px] max-h-[85vh] overflow-y-auto">
            ...
            {!editGroup.permissions.includes('*') && (
              <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-2">
                ...
                <button onClick={saveGroup} disabled={saving} className="px-5 py-2 text-xs text-white bg-blue-600 rounded-lg hover:bg-blue-700 font-semibold flex items-center gap-1.5 disabled:opacity-60">
        ...
        <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center p-4" onClick={() => setShowAddGroup(false)}>
          <div onClick={e => e.stopPropagation()} className="bg-white rounded-2xl border border-gray-200 shadow-2xl w-[400px]">
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-2">
              ...
              <button onClick={addGroup} disabled={saving} className="px-5 py-2 text-xs text-white bg-blue-600 rounded-lg hover:bg-blue-700 font-semibold disabled:opacity-60">{saving ? 'Đang tạo...' : 'Tạo nhóm'}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center p-4" onClick={() => setEditEmpId(null)}>
          <div onClick={e => e.stopPropagation()} className="bg-white rounded-2xl border border-gray-200 shadow-2xl w-[700px] max-h-[85vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                ...
                <select value={empRole} onChange={e => setEmpRole(e.target.value as UserRole)}
                  ...
                  {groups.map((g: PermissionGroup) => <option key={g.name} value={g.name}>{g.label} — {g.description}</option>)}
                </select>
              </div>
              ...
              <div>
                ...
                <div className="overflow-x-auto border border-gray-200 rounded-lg">
                  <table className="w-full text-xs">
                    ...
                    <tbody>
                      {ALL_MODULES.map(m => {
                        const roleGroup = groups.find((g: PermissionGroup) => g.name === empRole)
                        return (
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-2">
              ...
              <button onClick={saveEmpPerms} disabled={saving} className="px-5 py-2 text-xs text-white bg-blue-600 rounded-lg hover:bg-blue-700 font-semibold flex items-center gap-1.5 disabled:opacity-60">
                <Check size={13}/> {saving ? 'Đang lưu...' : 'Lưu quyền'}
              </button>
            </div>
          </div>
        </div>
