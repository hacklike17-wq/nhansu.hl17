...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // tong_thuc_nhan is always auto-computed — block any edit
  ...
  // System columns: only allow calcMode + order updates (not name/type/formula)
  ...
  const col = await db.salaryColumn.update({
    ...
    data: {
      ...parsed.data,
      // Clear formula when switching type to 'number'
      formula: effectiveType === "formula" ? effectiveFormula : null,
    },
  })

  // Phase 08: create version snapshot when formula is updated
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Prevent deleting system columns + cross-company check
